...
__global__
void fill(double* x, const double value, const int n) {
    auto i = threadIdx.x + blockDim.x*blockIdx.x;
    if(i < n) {
        x[i] = value;
    }
...
__global__
void axpy(double* y, const double* x, const double alpha, const int n) {
    auto i = threadIdx.x + blockDim.x*blockIdx.x;
    if(i < n) {
        y[i] += alpha*x[i];
    }
...
__global__
void scaled_diff(double* y, const double alpha, const double* l, const double* r, const int n) {
    auto i = threadIdx.x + blockDim.x*blockIdx.x;
    if(i < n) {
        y[i] = alpha*(l[i] - r[i]);
    }
...
__global__
void scale(double* y, const double alpha, const double* x, const int n) {
    auto i = threadIdx.x + blockDim.x*blockIdx.x;
    if(i < n) {
        y[i] = alpha*x[i];
    }
...
__global__
void lcomb(double* y, const double alpha, const double* x, const double beta, const double* z, const int n) {
    auto i = threadIdx.x + blockDim.x*blockIdx.x;
    if(i < n) {
